...
export default function Question({
  ...
  // Make teapot API call when question 2 is rendered
  useEffect(() => {
    if (question.id === 2) {
      // Make the API call in the background
      fetch('/api/teapot')
        .then(response => {
          console.log(`Teapot API called for question 2! Status: ${response.status} - ${response.statusText}`);
          return response.text();
        })
        .then(text => {
          console.log('Teapot response:', text);
        })
        .catch(error => {
          console.error('Teapot API error:', error);
        });
    }
  }, [question.id]);
